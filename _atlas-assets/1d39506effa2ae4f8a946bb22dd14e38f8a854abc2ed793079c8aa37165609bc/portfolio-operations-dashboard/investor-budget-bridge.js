  ...
  if(new URLSearchParams(location.search).get('investorReader')==='1') {
    window.addEventListener('message',async event=>{
      ...
      try {
        ...
        const [adapter,matcher,communities,aliases,forecastStore]=await Promise.all([import('./features/canonical-finance.mjs?v=a5d92a10ce5c448c'),import('./features/financial-package.mjs?v=a378a0cb25083758'),central.readCommunitiesForAccess(),central.fetchJson('/atlas_community_aliases?active=eq.true&select=community_id,alias,active&limit=1000'),import('./features/reforecast-store.mjs?v=a31fb99b0826a753')]);
