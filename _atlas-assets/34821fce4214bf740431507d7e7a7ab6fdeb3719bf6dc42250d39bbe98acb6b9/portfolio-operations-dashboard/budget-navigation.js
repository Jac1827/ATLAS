...
(function(R){
 ...
 const go=A.go;
 let packageReview;
 A.openFinancialPackageReview=async function(scope){try{packageReview=await import('./features/financial-package-review.mjs?v=7b25f9faf6be13c0');await packageReview.openReview(scope);}catch(e){A.toast(e.message,'r');}};
 ...
 if(typeof window!=='undefined'&&window.parent!==window&&new URLSearchParams(location.search).get('investorReader')!=='1')import('./features/financial-close.mjs?v=7cc9a038b2cc2e9b').then(async m=>{
