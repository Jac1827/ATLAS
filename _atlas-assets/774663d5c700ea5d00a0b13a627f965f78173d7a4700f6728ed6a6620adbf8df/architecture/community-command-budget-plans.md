...
## Monthly goal persistence repair — September 22

Leasing & Occupancy Plan goals are separate from the task execution plans described above. The old goal editor wrote only to `communityCommandState` in the dashboard settings document and closed without awaiting persistence. That document can be replaced by refresh/import hydration, so it cannot be the authoritative store for approved goals.

`centralization/community-goals.sql` adds a separate monthly goal store keyed by canonical community UUID and `YYYY-MM`. Immutable goal records retain `recommended`, `draft`, and `approved` revisions. A small head row independently identifies each current record. Recommendations have their own revision counter, so recalculations neither replace human records nor invalidate a draft's expected revision. Approval atomically updates the approved head and clears the active draft pointer; the consumed draft remains in immutable history.

`atlas_save_community_goals` checks active Admin/executive authorization, community scope and module access. Human saves require an adjustment reason and effective date. The server supplies actor identity, approval timestamp and revision. Approval validates complete production counts and exact weekly totals, preserving zero as a number and unavailable values as null. Client request UUIDs make lost-response retries idempotent; stale expected revisions fail without replacing saved work. Clients have SELECT only, and all exposed tables use scoped RLS. A database trigger rejects mutations to historical records.

`features/community-goals.mjs` reads committed goal records directly. Save success requires an exact immutable record readback and a matching current head. A failed read, version conflict or intervening newer save is surfaced to the editor; it is not converted into an empty result or a success notification. Approval consumers use the same effective-dated approved history. Schema-version-4 task execution reports copy that approved record into their immutable snapshot and include it in report deduplication, so a later goal revision cannot change an earlier report or export.

Apply `community-goals.sql` after the existing Community Command and canonical-finance migrations. It does not automatically promote browser-local legacy goals or change operational uploads. Roll back UI code if necessary while retaining the goal tables and history. Local PostgreSQL and adapter tests are `tools/community-goals-db.test.cjs` and `tools/community-goals.test.mjs`; the browser harness exercises the actual editor handlers against the same schema without production data.
