import {readFinance} from './canonical-finance.mjs?v=c74aa79c6d9efbbe';
import {mountCloseControls,readYear,coverage,readRows} from './financial-close.mjs?v=7e682ad0daf3fce6';
 ...
 try{
  ...
  async function load(){const token=++epoch;result.replaceChildren();const cid=container.querySelector('[data-community]').value,p=container.querySelector('[data-period]').value;
   ...
   try{const [finance]=await readFinance(central,[cid],[p]);if(!alive()||token!==epoch)return;const policy=finance?.summary?.coveragePolicy;
    if(finance?.summary?.budgetVersion&&(!finance.summary.actualCloseVersion||policy?.fullMonthAllowed===false)){status.textContent='Reading the approved original budget for screen and exports…';const {mountBudgetReport}=await import('./canonical-budget-report.mjs?v=d14703fda5b65d09');await mountBudgetReport(result,central,cid,p,{isCurrent:()=>alive()&&token===epoch});if(!alive()||token!==epoch)return;status.textContent='Approved original budget. Actuals remain unavailable for this period; screen, PDF, CSV and Excel use the same retained canonical snapshot.';return;}
    ...
    if(closed){status.textContent='Reading the published version for screen and exports…';const {mountCloseReport}=await import('./financial-close-report.mjs?v=370472ed957925fb');await mountCloseReport(result,central,cid,p,{isCurrent:()=>alive()&&token===epoch});if(!alive()||token!==epoch)return;status.textContent='Published full-month actuals. Screen, PDF, CSV and Excel use the same retained canonical snapshot.';return;}
