...
Production migration application, production application deployment, post-deployment financial readback, and live multi-user signoff: **pending / not verified**. The branch is prepared for the established Cloudflare preview workflow; its run and preview URL are recorded in the pull request/handoff. A real authorized editor, approver, and denied-role session must verify the deployed lifecycle without approving actual financial plans merely as a smoke test. Available approved baselines, initial Doro scenario access, and eligible provider/utility history require per-community verification.

**Coordinated rollout required:** the old UI sends separate approve/lock actions that the new migration rejects, and it does not understand the new inherited-locked snapshot state. Do not install the migrations and leave the old application active through an extended preview cycle. The missing release prerequisite is an isolated ATLAS test environment or an agreed test community with two authorized accounts and one denied-role account, so deployment and authenticated acceptance can be coordinated without approving real financial plans. No production migration has been applied; activation remains pending this coordinated rollout and authenticated access. A read-only availability check also found no active canonical incentive plan or canonical bonus-eligible assignment proof; the deployed employee schema lacks the explicit bonus-eligibility field. Existing local plan defaults cannot fill this gap. Canonical plan/employee eligibility must be established through the separate governed People/Bonus setup before unpaid results can become payable. The gate handles the missing field safely and does not add or infer eligibility.
