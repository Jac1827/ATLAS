...
(function(R){
 ...
 const go=A.go;
 let packageReview;
 A.openFinancialPackageReview=async function(scope){try{packageReview=await import('./features/financial-package-review.mjs?v=238081094414b4c1');await packageReview.openReview(scope);}catch(e){A.toast(e.message,'r');}};
 ...
 if(typeof window!=='undefined'&&window.parent!==window&&new URLSearchParams(location.search).get('investorReader')!=='1')import('./features/financial-close.mjs?v=5158fa45d3c5951c').then(async m=>{
