...
(function(R){
 ...
 const go=A.go;
 let packageReview;
 A.openFinancialPackageReview=async function(){try{packageReview=await import('./features/financial-package-review.mjs?v=a5fa31b16a3c0c8e');await packageReview.openReview();}catch(e){A.toast(e.message,'r');}};
 const actuals=R.views.actuals;
 function sharedActualsPanel(){
  setTimeout(async()=>{const el=document.getElementById('shared-financial-comparison');if(!el)return;try{const m=await import('./features/financial-comparison.mjs?v=e7ec3b107d972c49');await m.mountComparison(el,{communityName:A.cp().property.name,year:A.year()});}catch(e){el.textContent=e.message;}},0);
 ...
 if(typeof window!=='undefined'&&window.parent!==window&&new URLSearchParams(location.search).get('investorReader')!=='1')import('./features/financial-close.mjs?v=fb03da0d88b68a73').then(async m=>{
