...
export function validateBudgetApproval(payload,cid,requestId){
...
export async function approveOriginalBudget({central,cid,payload,requestId,onStatus=()=>{}}){
 ...
 try{
  ...
  validateBudgetApproval(payload,cid,requestId);session();phase='queued';onStatus(budgetPublicationStatus(phase,'Submitting the reviewed source to central approval.',{requestId}));
...
export async function reviewOriginalBudget({R,central,cid,prop,year,onStatus=()=>{}}){
 ...
 const initialStart=months.indexOf(prop.fiscalYearBegins||'Jan')+1;
 ...
 const sourceEvidence=document.createElement('p');sourceEvidence.textContent=baseline.sourceHash?`Source SHA-256 (${baseline.sourceHashKind||'workbook_bytes'}): ${baseline.sourceHash}`:'The original workbook bytes are not retained in this older browser stage. Approval fingerprints the exact reviewed rows and records normalized_approved_rows as the hash basis.';sourceEvidence.style.overflowWrap='anywhere';effective.before(sourceEvidence);
 ...
 el.querySelector('[data-save]').onclick=async()=>{
  ...
  try{
   const sourceHash=baseline.sourceHash||await digest({sourceFile:baseline.sourceFile,sourceSheet:baseline.sourceSheet,effectiveDate:baseline.effectiveDate,rows});
   ...
   payload.effectiveDate=effectiveInput.value;payload.sourceHashKind=baseline.sourceHashKind||(baseline.sourceHash?'workbook_bytes':'normalized_approved_rows');payload.mappingVersion='budget-mapping-'+await digest({metricMappings:payload.metricMappings,rows:rows.map(r=>({glCode:r.glCode,nature:r.nature,group:r.group})),coverage,fiscalYear:payload.fiscalYear,fiscalStartMonth:payload.fiscalStartMonth});
