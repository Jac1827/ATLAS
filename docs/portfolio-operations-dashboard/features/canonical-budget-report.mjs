...
export async function readBudgetSnapshot(central,cid,period){
 ...
 return freezeSnapshot(structuredClone({communityId:cid,period,publicationId:record.publication_id||null,financialSnapshot:record.summary.financialSnapshot,budget,actualsReason:record.summary.coveragePolicy?.reason||record.summary.coverageReason||'No published full-month actuals are available for this period.'}));
}
