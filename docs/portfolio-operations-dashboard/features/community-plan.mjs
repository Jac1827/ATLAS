import {reportHtml} from './community-plan-report.mjs?v=0d9d659e2a655a0c';
...
export async function open(context) {
 ...
 try {
  const [rows,financial]=await Promise.all([
   ...
   context.central.fetchJson(`/atlas_command_financial_summaries?community_id=eq.${context.communityId}&period_key=eq.${context.period}&select=*&limit=1`,{signal:state.abort.signal}).catch(()=>[])
  ]);
  state.financial=financial[0];
  state.findings=state.financial ? await context.central.fetchJson(`/atlas_command_findings?community_id=eq.${context.communityId}&period_key=eq.${context.period}&publication_id=eq.${state.financial.publication_id}&select=*&limit=10`,{signal:state.abort.signal}) : [];
...
function render(state) {
 ...
 dialog.querySelector('[data-plan]').onsubmit=event=>{event.preventDefault();if(!canEdit)return;const data=new FormData(event.target);save(state,{...p,owner:data.get('owner'),executionOwner:data.get('executionOwner'),stage:data.get('stage')});};
