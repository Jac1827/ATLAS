...
export async function mountCloseControls(container,central,review){
 ...
 let operationalReady=false;button.disabled=true;const operational=document.createElement('section');panel.insertBefore(operational,button);try{await mountMonthEndReview(operational,central,review,{onReady:ready=>{operationalReady=ready;button.disabled=!ready;}});}catch(error){status.textContent=error.message;}
 if(heads.length){const reopen=document.createElement('button');reopen.textContent='Reopen closed period';panel.append(reopen);reopen.onclick=async()=>{if(reason.value.trim().length<5){status.textContent='Enter the formal reopen reason first.';return;}reopen.disabled=true;try{await central.rpc('atlas_reopen_actual_period',{p_community_id:review.community_id,p_period:review.period_key,p_expected_version_id:heads[0].version_id,p_reason:reason.value.trim(),p_request_id:crypto.randomUUID()});status.textContent=REOPENED_PERIOD_WARNING;invalidateFinanceReads(central);}catch(error){status.textContent=error.message;reopen.disabled=false;}};}
