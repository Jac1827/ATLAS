import {readFinance,readApprovedBudget,financialSummary,bonusEvidence} from './canonical-finance.mjs?v=a2d996d897d3850b';
...
export function installBuilder(R,central,resolve){
 ...
 R.variance.compute=function(state,calc,pid,year){const result=compute.apply(this,arguments),c=caches.get(pid+'|'+year);if(!c)return result;
  ...
  result.canonicalBudgetVersion=approved?envelope?.budgetVersion||null:null;result.canonicalBudgetVersions=approved?c.budget?.periodVersions||{}:{};result.canonicalCoverage=c.coverage;result.canonicalVersions=c.versions.map(v=>v.version_id);return result;
 };
