import {pdfItemsToText,parseComparisonLines,reconcileComparison,classifyStatement} from './financial-package.mjs?v=49ea086d6d07f300';
...
export async function readPackage(file,{signal,onProgress=()=>{}}={}) {
 ...
 try {
  ...
  for(let number=1;number<=Math.min(pdf.numPages,64);number++) {
   ...
   page.cleanup();text='';
   // GL transactions and supporting pages are deliberately outside monthly-close ingestion.
   if(type==='gl_detail'&&parts.length)break;
...
function readWorkbook(buffer,file,hash,{signal,onProgress}) {
 return new Promise((resolve,reject)=>{
  const worker=new Worker(new URL('./financial-workbook-worker.mjs?v=bcf0b3d21876f138',import.meta.url),{type:'module'});
  ...
  const finish=(error,result)=>{if(settled)return;settled=true;clearTimeout(timer);worker.terminate();signal?.removeEventListener('abort',cancel);error?reject(error):resolve({...result,sourceFile:file.name,sourceHash:hash,sourceBytes:file.size,scope:'Budget Comparison only; reference worksheets are excluded from posting rows.'});};
