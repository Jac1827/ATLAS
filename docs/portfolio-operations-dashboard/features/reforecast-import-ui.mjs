...
const reviewFields=['assignment','periods','scenario','currency','accountChoices','reason','confirmed','calendar','inputReviews','integrityReviews','inputReason','requestId','priorVersionId','destination','forecastName','recordType','reviewId','currencyAliasConfirmed','scopeSelectionKey','scopedFingerprint','scopedUploadRequestId'];
...
function renderProperty(state){
 ...
 body.querySelector('[data-save-evidence]').onclick=async()=>{
  ...
  try{
   ...
   if(!state.aliases)state.aliases=await state.central.fetchJson('/atlas_community_aliases?active=eq.true&select=community_id,alias,active&limit=1000');guard(state);const identity=resolveImportCommunity({fileName:state.evidence.source.fileName,embeddedNames:state.evidence.metadata?.entities||[],selectedCommunityId:selected,confirmed,reason},state.communities,state.aliases);if(identity.status!=='resolved'||identity.communityId!==selected)throw Error(identity.reason||'Select and confirm the correct canonical community mapping.');state.assignment.identityEvidence=identity.evidence;const validation=validateReforecastPropertyAssignment(state.assignment,state.communities.map(communityId));if(!validation.valid)throw Error(validation.issues.map(item=>item.message).join('\n'));
...
async function loadSource(state){
 ...
 state.verifiedCalendar=await state.central.fetchJson('/rpc/atlas_read_budget_calendar',{method:'POST',body:JSON.stringify({p_community_id:state.assignment.communityId})});guard(state);if(!state.verifiedCalendar?.verified)throw Error(state.verifiedCalendar?.reason||'Verify the community calendar in Community Settings before import.');if(!state.calendar||state.calendar.basis!==state.verifiedCalendar.basis||state.calendar.startMonth!==state.verifiedCalendar.startMonth)state.calendar={...state.verifiedCalendar,confirmed:false,periods:[...state.periods],scenario:state.scenario};state.source=await readSourceBundle(state.central,{communityId:state.assignment.communityId,periods:state.periods});guard(state);if(state.purpose==='original_budget')state.source={...state.source,actuals:{...state.source.actuals,cutoffPeriod:null},intakePurpose:'original_budget'};
...
function readReviewFields(state){
 ...
 state.forecastName=root.querySelector('[data-forecast-name]')?.value||state.forecastName;state.recordType=root.querySelector('[data-record-type]')?.value||state.recordType||'reforecast';
...
function renderMapping(state){
 ...
 ${state.purpose==='original_budget'?'':`<p class="error"><strong>UNSAVED</strong> — workbook evidence is saved; reviewed values become shared only after the working forecast is saved and read back.</p><div class="rfi-grid"><label>Destination scenario<select data-destination><option value="new" ${state.destination==='new'?'selected':''}>Create New Draft</option>${state.currentDraft?`<option value="current" ${state.destination==='current'?'selected':''}>Update Existing Draft: ${esc(state.currentDraft.name)}</option>`:''}</select></label><label>Record type<select data-record-type>${selectOptions([{value:'initial_budget',label:'Initial Budget'},{value:'revised_budget',label:'Revised Budget'},{value:'reforecast',label:'Reforecast'}],state.recordType||'reforecast')}</select></label><label>Working forecast name<input data-forecast-name value="${esc(state.forecastName)}"></label></div>`}<div class="rfi-grid"><label>Exact workbook source scenario<select data-scenario>${selectOptions(scenarios.map(value=>({value,label:value+(/^Actual\b/i.test(value)?' — evidence only':'')})),state.scenario)}</select></label><label>Reporting currency<select data-currency>${selectOptions(['USD','CAD','EUR','GBP','AUD','NZD','JPY'].map(value=>({value,label:value})),state.currency)}</select></label></div>
