  ...
  if(new URLSearchParams(location.search).get('investorReader')==='1') {
    window.addEventListener('message',async event=>{
      ...
      try {
        ...
        const [adapter,matcher,communities,aliases,forecastStore]=await Promise.all([import('./features/canonical-finance.mjs?v=60c13a0342f297e2'),import('./features/financial-package.mjs?v=b43f129095c7fac2'),central.readCommunitiesForAccess(),central.fetchJson('/atlas_community_aliases?active=eq.true&select=community_id,alias,active&limit=1000'),import('./features/reforecast-store.mjs?v=50949947b3848714')]);
        ...
        for(const community of selected){
          ...
          for(const row of records.filter(r=>r.community_id===community.community_id)){
            ...
            for(const [key,value] of Object.entries(s)){
              ...
              for(const basis of ['actual','budget'])if(adapter.number(value[basis])!==null){item[basis]=adapter.number(value[basis]);item.sources[basis]=`${basis==='actual'?s.actualSource:s.budgetSource} / version ${basis==='actual'?s.actualCloseVersion:s.budgetVersion} / ${s.registryVersion} / ${row.period_key}`;item.definitions[basis]=key+' / approved canonical metric registry';}
            }
