  ...
  if(new URLSearchParams(location.search).get('investorReader')==='1') {
    window.addEventListener('message',async event=>{
      ...
      try {
        ...
        const [adapter,matcher,communities,aliases,forecastStore]=await Promise.all([import('./features/canonical-finance.mjs?v=491d9382e664ca55'),import('./features/financial-package.mjs?v=a378a0cb25083758'),central.readCommunitiesForAccess(),central.fetchJson('/atlas_community_aliases?active=eq.true&select=community_id,alias,active&limit=1000'),import('./features/reforecast-store.mjs?v=da9a2923e4b2392f')]);
