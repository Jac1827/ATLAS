...
(function (root) {
  ...
  const formulas = {
    ...
    dscr:['noi','debtService','divide'], costPerLead:['marketingExpense','guestCards','divide'], costPerLease:['marketingExpense','leasesSignedActual','divide'], expensePerUnit:['expenses','totalUnits','divide'], noiPerUnit:['noi','totalUnits','divide']
  };
  ...
  function build({community,period,record={},config={},draft={},prior=null,now=new Date()}) {
    ...
    rows.filter(r=>r.material&&!draft.drivers?.some(d=>d.metric===r.id&&d.classification==='confirmed'&&d.evidence&&d.owner&&d.action)).forEach(r=>issues.push({owner:draft.metricOwners?.[r.id]||'Owner unassigned',metric:r.label,issue:'Material movement needs a confirmed explanation, evidence, owner and action'}));
    ...
    return {schemaVersion:1,community,period,connections:record.investorConnections||[],segments:record.investorSegments?.[period]||[],generatedAt:now.toISOString(),partial,rows,issues,audit,draft:JSON.parse(JSON.stringify(draft)),config:{housingTypes:config.housingTypes||[]},prior:prior?{period:prior.period,reviewedAt:prior.reviewedAt,actions:prior.draft?.actions||[],risks:prior.draft?.risks||[]}:null};
  }
