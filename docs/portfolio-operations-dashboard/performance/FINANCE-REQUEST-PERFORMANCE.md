...
Successful sample plans reported zero shared disk-read and temporary-read blocks. This does not establish that other workloads have no I/O cost. The portfolio/year input was derived from the deployed client source; it was not represented as a captured browser payload. The [sanitized production baseline plans](evidence/issue-12/finance-production-baseline-plans.json) retain query shape, EXPLAIN/ANALYZE nodes, timings and buffer counts without actor identifiers or financial values. Full private diagnostic artifacts remain in ignored `output/issue12-baseline/`.


A separate local PGlite/WASM plan used the complete restored schema and its existing budget-projection trigger to generate 336 synthetic summaries (28 communities × 12 months). With the same rows and authenticated synthetic role, the old portfolio/year reader took 13,182.25 ms and the candidate took 7,815.35 ms. The one-community/month observations were 68.912 ms and 78.218 ms respectively. These single local observations do not predict production latency: the portfolio request remains expensive, and selecting only the current UI scope is essential. The [synthetic before/after plans](evidence/issue-12/finance-local-comparison-plans.json) retain all four observations and the parameter-only query template; no production source values are included. Temporary block work increases in this local plan, so the lower total elapsed time must not be described as a universal I/O improvement.
