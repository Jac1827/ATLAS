...
`node tools/scoped-workspace-projection-db.test.cjs` passes against the complete 76-migration local replay. Coverage includes scoped and full roles, anonymous/disabled/module-category locks, self-service-only context, scope changes, raw-document and private-helper denial, ambiguous/inactive aliases, UUID contradiction, private-field removal, exact full-body hash, zero/null/provenance and missing/stale parent sources. These are synthetic local checks; no production migration or role test has been performed for this change.

Deploy with the matching client adapter and verified materialized projection. Rollback can restore the previous app and revoke this new RPC's authenticated execute grant in a forward migration. Keep archive/projection records and existing document RLS intact; do not widen archive access as a fallback.
