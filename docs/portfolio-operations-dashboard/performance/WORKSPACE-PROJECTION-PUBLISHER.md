...
Read-only preflight observed deployed PostgREST 14.5 and no `pgrst.db_hoisted_tx_settings` override in role/database catalogs. An environment-only override cannot be ruled out through those catalogs. Local database tests validate the function declaration; actual HTTP execution remains the final check that hoisting applies.
...
Local success is not production acceptance. Deploy the migration before using the new caller, verify the RPC/schema cache, then perform one controlled authorized materialization and exact readback. An ambiguous HTTP outcome must be resolved by reading the exact source-bound key before retrying. No automatic retry loop or general timeout increase is part of this change.
