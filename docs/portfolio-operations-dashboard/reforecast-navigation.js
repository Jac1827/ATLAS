...
(function(R){
 ...
 for(const [id,label,mode] of routes){A.VIEWS.push({id,label});R.views[id]=()=>{setTimeout(async()=>{const el=document.getElementById('atlas-reforecast-workspace');if(!el)return;try{const m=await import('./features/reforecast-ui.mjs?v=11666d231850c3cf');if(el.isConnected)await m.mountReforecast(el,{R,mode});}catch(e){el.textContent='Reforecast workspace could not load: '+e.message;}},0);return '<div id="atlas-reforecast-workspace" role="region" aria-label="Governed reforecast"><p>Loading shared reforecast workspace…</p></div>';};}
 ...
 const clearScope=()=>{delete R.reforecastSources;delete R.reforecastPropertyAssignments;import('./features/reforecast-legacy-bridge.mjs?v=418229d306218ee6').then(m=>m.clearLegacyReforecastCache());import('./features/reforecast-ui.mjs?v=11666d231850c3cf').then(m=>m.clearReforecastSession());};
 ...
 window.addEventListener('message',e=>{if(e.origin!==location.origin||e.source!==window.parent)return;if(e.data?.type==='atlas-reforecast-navigate'){A.go(e.data.view||'reforecast');if(e.data.scenarioId||e.data.reviewId)setTimeout(async()=>{try{const m=await import('./features/reforecast-ui.mjs?v=11666d231850c3cf'),container=document.getElementById('atlas-reforecast-workspace');if(!container)return;await m.mountReforecast(container,{R,mode:'approvals'});await m.openBudgetReview(e.data);}catch(error){A.toast(error.message,'r');}},0);}});
 document.addEventListener('atlas-create-reforecast',async event=>{try{const m=await import('./features/reforecast-ui.mjs?v=11666d231850c3cf');A.go('reforecast');const container=document.getElementById('atlas-reforecast-workspace');if(!container)throw Error('The Working Drafts view is unavailable.');await m.mountReforecast(container,{R,mode:'workspace'});await m.createRecommendedReforecast(event.detail);document.querySelectorAll('dialog[open]').forEach(dialog=>dialog.close());A.go('reforecast');}catch(error){A.toast(error.message,'r');}});
