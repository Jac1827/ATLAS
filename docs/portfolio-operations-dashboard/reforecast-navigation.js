...
(function(R){
 ...
 for(const [id,label,mode] of routes){A.VIEWS.push({id,label});R.views[id]=()=>{setTimeout(async()=>{const el=document.getElementById('atlas-reforecast-workspace');if(!el)return;try{const m=await import('./features/reforecast-ui.mjs?v=658d8ca2ed4e8f45');if(el.isConnected)await m.mountReforecast(el,{R,mode});}catch(e){el.textContent='Reforecast workspace could not load: '+e.message;}},0);return '<div id="atlas-reforecast-workspace" role="region" aria-label="Governed reforecast"><p>Loading shared reforecast workspace…</p></div>';};}
 R.budgetNavigation?.groups.find(g=>g[0]==='forecast')?.[2].push(...routes.map(r=>r[0]));
 import('./features/reforecast-legacy-bridge.mjs?v=cbf163abfa88444f').then(m=>m.installLegacyReforecastBridge(R)).catch(e=>A.toast('Scenario calculator failed: '+e.message,'r'));
 const clearScope=()=>{delete R.reforecastSources;delete R.reforecastPropertyAssignments;import('./features/reforecast-legacy-bridge.mjs?v=cbf163abfa88444f').then(m=>m.clearLegacyReforecastCache());import('./features/reforecast-ui.mjs?v=658d8ca2ed4e8f45').then(m=>m.clearReforecastSession());};
