...
(function(R){
 ...
 for(const [id,label,mode] of routes){A.VIEWS.push({id,label});R.views[id]=()=>{setTimeout(async()=>{const el=document.getElementById('atlas-reforecast-workspace');if(!el)return;try{const m=await import('./features/reforecast-ui.mjs?v=61c6e85f031ff85b');if(el.isConnected)await m.mountReforecast(el,{R,mode});}catch(e){el.textContent='Reforecast workspace could not load: '+e.message;}},0);return '<div id="atlas-reforecast-workspace" role="region" aria-label="Governed reforecast"><p>Loading shared reforecast workspace…</p></div>';};}
 ...
 const clearScope=()=>{delete R.reforecastSources;delete R.reforecastPropertyAssignments;import('./features/reforecast-legacy-bridge.mjs?v=0dbab913e5e81690').then(m=>m.clearLegacyReforecastCache());import('./features/reforecast-ui.mjs?v=61c6e85f031ff85b').then(m=>m.clearReforecastSession());};
