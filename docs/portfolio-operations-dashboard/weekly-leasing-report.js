...
(function(root, factory) {
  ...
  function document(data) {
    ...
    const professionals = agents => table(['Professional','Assigned records','Application Funnel · current status','Completed this week','Dated approvals this week','Dated lease signings this week'],agents.map(r=>[r.name,r.assigned,funnel(r),r.completed,r.approvals,r.leases])) + `<div class="individuals">${agents.map(r=>`<details class="person-details"><summary>${esc(r.name)} · application status and dates (${r.assigned})</summary>${table(['Application ID','Current status','Partially completed date','Completed date','Approval date','Lease signing date','Source data through'],r.applications.map(a=>[a.id,a.status,a.partial||'Not supplied',a.completed||'Not supplied',a.approved||'Not supplied',a.signed||'Not supplied',a.asOf||'Not supplied']))}</details>`).join('')}</div>`;
