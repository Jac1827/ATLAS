...
export class EvictionCaseState {
  ...
  async handle(request){
    ...
    if(action==='get'){const s=await read();return json({ok:true,...s,attorneys:await this.state.storage.get('attorneys')||[],securePortalUrl:await this.state.storage.get('securePortalUrl')||'',emailAvailable:!!this.env.EMAIL&&!!this.env.ATLAS_DLR_FROM_EMAIL});}
    if(action==='attorneys'){
      const emails=payload.emails;if(!Array.isArray(emails)||!emails.length||emails.length>10||emails.some(e=>!/^\S+@[^\s@]+\.[^\s@]+$/.test(e)))throw error('Enter valid attorney email addresses.');
      if(payload.securePortalUrl&&!/^https:\/\//.test(payload.securePortalUrl))throw error('Use an approved HTTPS secure portal URL.');
      await this.state.storage.put('attorneys',emails);await this.state.storage.put('securePortalUrl',payload.securePortalUrl||'');return json({ok:true});
    }
    if(action==='upload'){
    ...
    if(action==='send'){
      if(!this.env.EMAIL||!this.env.ATLAS_DLR_FROM_EMAIL)throw error('Email sending is unavailable. Connect the authorized ATLAS email service; your draft is saved.',503);
      ...
      try{
        const result=await this.env.EMAIL.send({to:packet.draft.to,from:{email:this.env.ATLAS_DLR_FROM_EMAIL,name:'RISE Central Services'},subject:packet.draft.subject,text:packet.draft.body,attachments});
        if(!result?.messageId)throw error('The provider returned no message ID; submission needs reconciliation.');
        const s=await mutate(s=>{s.draft.status='sent';s.draft.sentAt=now();s.draft.providerMessageId=result.messageId;s.draft.deliveryStatus='unconfirmed';audit(s,'Sent to attorney',{providerMessageId:result.messageId,to:s.draft.to,attachments:s.draft.manifest});});return json({ok:true,...s});
      }catch(e){
