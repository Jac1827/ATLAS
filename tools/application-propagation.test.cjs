...
if(box){
 ...
 const month={},lineage=[];
 Object.assign(context,{getResolvedTotalUnitsForRecord:()=>320,dataImportShouldApplyCurrentMetric:()=>true,normalizeSavedCommunityRecord:(name,r)=>r||{},dataImportUpsertFloorPlan:()=>{},getWritableMonthlyPeriodEntries:()=>({historyEntry:month}),dataImportApplyMetric:(record,plan,result,name,period,target,value,field)=>{if(value===null||value===undefined)return false;assert(context.applyValueToRecord(record,period.monthIdx,target,value,period.year));lineage.push({currentState:true,reportType:'box_score',communityName:name,periodKey:period.periodKey,atlasField:field,importedValue:value,sourceFile:plan.name});return true;}});
 ...
 assert.equal(month.denied,1);assert.equal(month.cancelled,undefined,"Missing standalone cancellation total must not become zero");assert.equal(month.leasesSignedActual,4);assert.equal(month.applications,7);assert.equal(month.applicationsApproved,5);assert.equal(month.tours,26);assert.equal(month.leasedSnapshot,265);assert.equal(month.exposureAdjustedOccupancyPct,241/320*100);assert.equal(month.sourceLeasedUnits,265);assert.equal(month.exposureUnits,79);assert.equal(month.leasedOccupancyPct,82.8125);
 ...
 const explicitZeroEntries=entries.map((entry,index)=>index===0?{...entry,row:{...entry.row,cancelled_applications:0}}:entry);
 context.dataImportApplyGroupedSnapshot({communityName:'Sereno',period:{periodKey:'2026-09',year:2026,monthIdx:8},entries:explicitZeroEntries},{reportType:'box_score',name:'Explicit-zero fixture'},{issues:[],formulas:[],destinations:new Set()});
 assert.equal(month.cancelled,0,'An explicitly supplied cancellation zero must be preserved');
...
assert.equal(cancelled.values.cancelled_applications,undefined);assert.equal(cancelled.values.applications_completed_cancelled,3);assert.equal(cancelled.values.applications_approved_cancelled,2);assert.equal(cancelled.values.applications,10);assert.equal(cancelled.values.leases_completed,4);assert.equal(cancelled.locators.cancelled_applications,undefined);
