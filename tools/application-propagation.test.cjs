...
if(box){
 ...
 assert.equal(month.denied,1);assert.equal(month.cancelled,0);assert.equal(month.leasesSignedActual,4);assert.equal(month.applications,7);assert.equal(month.applicationsApproved,5);assert.equal(month.tours,26);assert.equal(month.leasedSnapshot,265);assert.equal(month.exposureAdjustedOccupancyPct,241/320*100);assert.equal(month.sourceLeasedUnits,265);assert.equal(month.exposureUnits,79);assert.equal(month.leasedOccupancyPct,82.8125);
