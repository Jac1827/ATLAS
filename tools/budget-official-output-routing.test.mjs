...
for(const name of actions){assert.equal(app[name]('value','segment'),false);assert.equal(app.view,'reports');const message=trace.messages.at(-1);assert.match(message,/Browser draft financial exports are disabled/);assert.match(message,/saved shared budget version/);assert.match(message,/PDF or Excel/);assert.match(message,/save it before generating a draft report/);}
...
for(const view of guardedViews){app.view=view;const rendered=R.views[view]();assert.match(rendered,/shared-financial-comparison/);assert.match(rendered,/snapshot fingerprint/);assert.match(rendered,/RBB\.app\.go\('reforecastgap'\)/,'Saved-version reporting remains reachable from every guarded financial view');assert.match(rendered,/saved budget version.*PDF.*Excel/);assert.match(rendered,/working revisions.*drafts/);assert.doesNotMatch(rendered,/No actuals loaded yet|Download the budget book|Legacy scenario comparison/);const top=app.renderTop();assert.match(top,/Shared financial records/);assert.doesNotMatch(top,/Effective gross income|Browser draft NOI|Publish to ATLAS/);const nav=app.renderNav();assert.match(nav,/Shared financial records/);assert.doesNotMatch(nav,/Locked original|Local editing lock|high-priority.*checks/);}
