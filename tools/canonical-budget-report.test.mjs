...
import {withEffectiveBaseline} from '../docs/portfolio-operations-dashboard/features/canonical-finance.mjs';
...
base.summary=withEffectiveBaseline(base.summary,{communityId:cid,period,status:'unavailable',reason:'Fixture has no verified active-baseline mapping'});
...
closed.summary=withEffectiveBaseline(closed.summary,base.summary.effectiveBaseline);
...
const actual=await readCloseSnapshot(session({record:closed}),cid,period),combined=closeReportRows(actual);assert.equal(combined.length,5);const byGl=new Map(combined.map(r=>[r.GL,r]));assert.equal(byGl.get('004110').Actual_minus_budget,0);assert.equal(byGl.get('004190').Actual_minus_budget,-5);assert.equal(byGl.get('006210').Original_budget,null);assert.equal(byGl.get('006210').Actual_minus_budget,null);assert.equal(byGl.get('007110').Actual,null);assert.equal(byGl.get('008110').Original_budget,null);assert(combined.every(r=>r.Snapshot===financeSnapshot(closed.summary,closed.publication_id).fingerprint));assert(closeReportHtml(actual).includes('Approved original budget'));assert(closeReportCsv(actual).includes('"Original_budget","Active_baseline","Actual_minus_active_baseline"'));
