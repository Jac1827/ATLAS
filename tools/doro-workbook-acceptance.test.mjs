...
const prospective=clone(publication);for(const period of periods)prospective.snapshot.lines.push({period,accountCode:'6501',nature:'expense',placement:'above_noi',forecast:0,originalBudget:null,actual:null,baselineDisposition:{kind:'no_original_budget_row',confirmed:true,reviewedBy:'test-actor',reviewedAt:'2026-09-25T00:00:00Z',reason:'New source GL has no original budget row',originalBudgetVersionIds:['test-original']}});
assert.equal(reconcilePublicationTotals(prospective).length,periods.length,'Explicit absent original rows retain null while preserving original budget totals');
const presentBlank=clone(prospective);delete presentBlank.snapshot.lines.at(-1).baselineDisposition;assert.throws(()=>reconcilePublicationTotals(presentBlank),/originalBudget expenses/,'A present but blank original amount cannot disappear from reconciliation');
