...
const document={readyState:'loading',addEventListener(){}};const window={addEventListener(){},ATLAS_CENTRAL:{getAccessContextKey:()=>actor}};
...
const getAtlasDashboardViewWidgets=()=>[],getAtlasActiveDashboardView=()=>({}),buildAtlasDashboardWidgetSnapshot=()=>snapshot;
...
(async()=>{
 ...
 assert.equal(await task,true);assert.ok(order.indexOf('finished')>order.indexOf('prepare'));assert.ok(order.indexOf('rendered')>order.indexOf('finished'));
