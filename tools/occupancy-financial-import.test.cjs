...
const c={console,Date,Map,Set,window:{AtlasFinancialPublication:F},savedData:{Test:{}},dataImportRuntimeCurrentLineageIndex:null,dataImport2State:{lineage:[]},MONTHS:Array(12).fill('Month')};
...
for(const change of [{occupied_units:50},{occupied_units:45.5},{occupied_units:91},{physical_occupancy:'70%'},{rentable_units:101}]){r=apply([{...row,...change}]);assert.equal(r.month.occupiedSnapshot,10);assert.equal(r.month.rentableUnits,20);assert(r.result.issues.some(x=>x.title==='Occupancy snapshot held'));}
...
r=apply([row],'box_score',{communityPropertyType:'Student Housing'});assert.equal(r.month.occupiedSnapshot,10,'Unit inventory is not published as beds');
...
r=apply([{actual_charges:100,gross_potential_rent:150},{actual_charges:-10,gross_potential_rent:50}],'rent_roll');assert.equal(r.month.actualCharges,90);assert.equal(r.month.grossPotentialRent,200);assert.equal(r.month.economicOccupancyPct,45);assert(c.dataImport2State.lineage.some(l=>l.atlasField==='actual_charges'));
...
assert.equal(r.record.financialLedger['2026-09'].length,2);assert.equal(r.record.financialBudgetLedger['2026-09'].length,2);let summary=c.getFinancialSummaryForMonth(r.record,8,2026);assert.equal(summary.noiActual,60);assert.equal(summary.noiBudget,70);assert.equal(summary.noiVariance,-10);
