...
const c = { console, Date, Map, Set, window: {}, MONTHS: ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'], dataImport2State: {lineage: [], canonicalRecords: []}, communityCommandState: {} };
...
let rows=c.buildCommunityCommandLeasingPlanRows(model);
for(let i=0;i<9;i++) {
  close(rows[i].beginningPct,(78+i*2)/188*100);
  if(i<8) close(rows[i].endingForecastPct,rows[i].beginningPct);
}
// Missing history remains missing even with skipped schedule zeros and current occupied data.
record.monthlyData[1]=c.defaultMonthly()[1];
rows=c.buildCommunityCommandLeasingPlanRows(model);
assert.equal(rows[1].beginningPct,null); assert.equal(rows[1].endingForecastPct,null); assert.equal(rows[1].status,'Missing');
assert(c.renderCommunityCommandLeasingPlan(model).includes('title="Missing period snapshot">Missing'));
...
assert.equal(c.buildCommunityCommandLeasingPlanRows(zeroModel)[1].endingForecastPct,0);
...
close(c.buildCommunityCommandLeasingPlanRows(model)[0].beginningPct,58/178*100);
...
// Approved economic occupancy uses signed period rent charges / positive GPR.
const financial={reportYear:2026,monthlyData:[],monthlyHistoryByPeriod:{'2026-01':{actualCharges:80,grossPotentialRent:100,economicOccupancyPct:99},'2025-12':{actualCharges:70,grossPotentialRent:100}},financialLedger:{'2026-01':[{actual:999,budget:10}]}};
let economic=c.getCommunityCommandEconomicOccupancyData(financial,0,2026);
assert.equal(economic.mtdPct,80);assert.equal(economic.priorPct,70);assert.equal(economic.variance,10);
for(const [charges,gpr,expected] of [[0,100,0],[-5,100,-5],[null,100,null],['',100,null],[80,0,null],[80,-1,null],[80,null,null]]) {
 Object.assign(financial.monthlyHistoryByPeriod['2026-01'],{actualCharges:charges,grossPotentialRent:gpr});
 assert.equal(c.getEconomicOccupancyForPeriod(financial,0,2026).pct,expected);
}
assert.equal(c.getEconomicOccupancyForPeriod(financial,0,2025).pct,null);
console.log('PASS approved economic occupancy, signed and zero charges, invalid denominators, exact period, prior-year comparison, no GL budget substitution.');
