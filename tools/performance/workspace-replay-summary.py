...
for g in x['results']:
    ...
    for tab,label in [(0,'Home'),(8,'Reports'),(7,'Import'),(2,'Command'),(9,'Bonus')]:
        ...
        d['navigation'][label]={'firstLoopMs':budget(stats([r['ms'] for r in first]),1000 if mobile else 500),'repeatedMs':budget(stats([r['ms'] for r in repeated]),400 if mobile else 200),'repeatedLongestTaskMs':budget(stats([r['longestTask'] for r in repeated]),200 if mobile else 100),'repeatedRenderCount':stats([r['renders'] for r in repeated]),'apiRequests':stats([r['apiRequests'] for r in repeated])}
    heap=g['heap'];start=heap['before']['usedSize'];end=heap['after']['usedSize'];sampled=[r.get('heap',0) or 0 for r in g['startup']+g['navigation']]
    d['memory']={'beforeRepeatedLoopsMiB':start/MiB,'afterRepeatedLoopsMiB':end/MiB,'retainedGrowthMiB':(end-start)/MiB,'retainedGrowthPass':end-start<=10*MiB,'steadyPass':end<=150*MiB,'peakSampledMiB':max(sampled)/MiB,'peakPass':max(sampled)<=250*MiB,'domBefore':heap['domBefore'],'domAfter':heap['dom']}
...
reference_parity={k:reference_complete and len({json.dumps(g['parity'][k],sort_keys=True)for g in repaired_reference})==1 for k in keys}
...
lines+=['','Shared operating-data parity across all builds: '+str(comparison['sharedOperatingDataAllBuilds'])+'. Baseline/repaired report and data parity: '+str(reference_complete and all(reference_parity.values()))+'. Historical Community Progress differences remain separately recorded in summary.json.','','Limits:']+['- '+v for v in x['limits']]
