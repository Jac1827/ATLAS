...
try{
 await page.route('**/__api/rpc/atlas_month_end_queue',route=>route.fulfill({status:200,contentType:'application/json',body:'[]'}));await page.goto(url);await page.locator('[data-community]').selectOption(cid);await idle();
 ...
 await page.locator('[data-workspace-view=status]').click();await idle();for(const type of ['html','pdf','xlsx']){const pending=page.waitForEvent('download');await page.locator('[data-digest-export="'+type+'"]').click();const download=await pending;await download.saveAs('output/playwright/forecast-builder/'+download.suggestedFilename());}

 assert.deepEqual(errors,[]);await deniedContext.close();await otherContext.close();
 console.log('PASS browser: full-month baseline setup, real SQL save/readback, two STR months with takeback, two independent sessions, mobile layout, denied Regional approval, atomic VP publication, active baseline, matching Admin/Regional report and Bonus target evidence, and PDF/XLSX/CSV downloads');
