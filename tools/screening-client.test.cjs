...
let readCount=0;w.ATLAS_CENTRAL.readScreeningImports=async()=>{readCount++;events['atlas-central-auth-change']();return rows;};await w.atlasRefreshScreening();assert.equal(readCount,1,'Same-user auth refresh cannot recursively request screening data');
let scopeReads=0;const originalScope=w.getAtlasApplicationScopeCommunityNames;w.getAtlasApplicationScopeCommunityNames=()=>{scopeReads++;return originalScope();};const many=Array.from({length:2000},(_,i)=>({applicationId:String(i),property:originalScope()[0],reportPeriodKey:'2026-09',sourceEffectiveAt:'2026-09-20',applicationStatus:'Application: Started'}));many[0].applicationStatus='Application: Approved';w.getAtlasApplicationIntelligenceData=()=>({records:many,history:many});w.getAtlasApplicationLineageReport();const cohortExport=w.getAtlasApplicationLineageExport()['Cohort Summary'][0];assert.equal(cohortExport.eligible,1);assert.equal(cohortExport.incomplete,1999);assert.equal(cohortExport.processing_eligible,0);assert(scopeReads<20,'Scope is resolved per calculation, not once per application');
