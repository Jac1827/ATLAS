...
(async()=>{
 ...
 for(const sheet of sheets){const m=apply(sheet.rows.map(sourceRow=>({row:sourceRow.values,sourceRow})),'box_score');for(const k of Object.keys(totals))totals[k]+=m[k]||0;if(sheet.sheetName==='RISE Citrus Ridge Townhomes'){assert.equal(m.occupiedSnapshot,184);assert.equal(m.leasedSnapshot,191);assert.equal(m.sourceLeasedUnits,191);assert.equal(m.rentableUnits,220);assert.equal(m.sourceTotalUnits,222);assert.equal(m.exposureUnits,41);assert.equal(m.leasedOccupancyPct,191/220*100);}if(sheet.sheetName==='RISE St. Augustine'){assert.equal(m.occupiedSnapshot,20);assert.equal(m.rentableUnits,272);assert.equal(m.sourceLeasedUnits,29);assert.equal(m.guestCards,24);assert(m.proformaRent>2800);assert(m.marketRent>2500);assert(m.nerActual>2100);}}
