...
c.getResolvedTotalUnitsForRecord=()=>20;
...
const ui={window:{getAtlasApplicationScopeCommunityNames:()=>{scopeCalls++;return ['Allowed'];},atlasApplicationCommunityInScope:()=>true,AtlasApplicationSources:bridge,AtlasApplicationLineage:{cohort:records=>records},AtlasApplicationAging:{latestRecords:records=>records},getAtlasApplicationIntelligenceData:()=>({records:Array.from({length:2500},(_,i)=>({applicationId:String(i),property:i%2?'Allowed':'Denied'}))}),addEventListener:()=>{}},Date};let scopeCalls=0;vm.createContext(ui);vm.runInContext(fs.readFileSync(path.join(root,'application-performance-ui.js'),'utf8'),ui);assert.equal(ui.window.getAtlasApplicationCommandRecords().length,1250);assert.equal(scopeCalls,1);ui.window.getAtlasApplicationScopeCommunityNames=()=>[];assert.equal(ui.window.getAtlasApplicationCommandRecords().length,0);
...
(async()=>{
 ...
 const inventory={'Anthem House':185,'Ivy & Elm':504,'Pilots Pointe at LSUS':388,'Prosper On Fayette':314,'RISE 34th':369,'RISE Bartram Park':297,'RISE Baymeadows':331,'RISE Citrus Ridge Townhomes':222,'RISE Doro':247,'RISE Florence Villa Townhomes':224,'RISE Sereno':320,'RISE St. Augustine':272,'The Preserve at Tech':588};
 for(const sheet of sheets){c.getResolvedTotalUnitsForRecord=()=>inventory[sheet.sheetName];const m=apply(sheet.rows.map(sourceRow=>({row:sourceRow.values,sourceRow})),'box_score');for(const k of Object.keys(totals))totals[k]+=m[k]||0;if(sheet.sheetName==='RISE Citrus Ridge Townhomes'){assert.equal(m.occupiedSnapshot,184);assert.equal(m.leasedSnapshot,191);assert.equal(m.sourceLeasedUnits,191);assert.equal(m.rentableUnits,220);assert.equal(m.sourceTotalUnits,222);assert.equal(m.exposureUnits,41);assert.equal(m.leasedOccupancyPct,191/220*100);}if(sheet.sheetName==='RISE St. Augustine'){assert.equal(m.occupiedSnapshot,20);assert.equal(m.rentableUnits,272);assert.equal(m.sourceLeasedUnits,29);assert.equal(m.guestCards,24);assert(m.proformaRent>2800);assert(m.marketRent>2500);assert(m.nerActual>2100);}}
 ...
 assert.equal(ds.flatMap(s=>s.rows).length,224,'Include the valid receivable without an assigned unit');
 assert.equal(ds.flatMap(s=>s.rows).filter(r=>r.values.last_delinquency_note).length,119,'All original delinquency notes survive canonical extraction');
